...
public:
	HashLoader(HashManager::HashStore& aStore, const CountedInputStream<false>& aCountedStream, uint64_t aFileSize, ProgressFunction aProgressF) :
		store(aStore),
		countedStream(aCountedStream),
		fileSize(aFileSize),
		progressF(aProgressF)
	{ }

	void startTag(const string& name, StringPairList& attribs, bool simple);
	void endTag(const string& name);
	int64_t readDataBytes = 0;
