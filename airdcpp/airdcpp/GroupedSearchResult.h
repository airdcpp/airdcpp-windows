...
namespace dcpp {
	...
	public:
		...
		BundleAddInfo downloadFileHooked(const string& aTargetDirectory, const string& aTargetName, Priority p);

		// Selects the best individual folders to download and queues them
		// Throws if none of the children could not be queued
		DirectoryDownloadList downloadDirectoryHooked(const string& aTargetDirectory, const string& aTargetName, Priority p);
