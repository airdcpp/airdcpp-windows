/* 
Since WTL does not support changing direction for the Splitter, which is very sad :(
Here's a little extended implementation of the Splitter class.
*/
...
public:
	...
	static HCURSOR m_hCursor;		//Horizontal size cursor
	static HCURSOR m_vCursor;		//Vertical size cursor
	...
	CSplitterEx() :
			...
			m_nSinglePane(SPLIT_PANE_NONE), t_bVertical(true)
	{
		...
		if(m_hCursor == NULL || m_vCursor == NULL) 
		{
			...
			if(m_vCursor == NULL)
				m_vCursor = ::LoadCursor(NULL, IDC_SIZEWE);
	...
	{
		dcassert(nPane == SPLIT_PANE_LEFT || nPane == SPLIT_PANE_RIGHT || nPane == SPLIT_PANE_NONE);
...
// Splitter operations
	void SetSplitterPanes(HWND hWndLeftTop, HWND hWndRightBottom, bool bUpdate = true)
	{
		...
		dcassert(m_hWndPane[SPLIT_PANE_LEFT] == NULL || m_hWndPane[SPLIT_PANE_RIGHT] == NULL || m_hWndPane[SPLIT_PANE_LEFT] != m_hWndPane[SPLIT_PANE_RIGHT]);
		if(bUpdate)
			UpdateSplitterLayout();
	}
	...
	{
		dcassert(nPane == SPLIT_PANE_LEFT || nPane == SPLIT_PANE_RIGHT);
	...
	LRESULT OnMouseMove(UINT /*uMsg*/, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
	{
		...
		else		// not dragging, just set cursor
		{
			if(IsOverSplitterBar(xPos, yPos))
				::SetCursor(t_bVertical ? m_vCursor : m_hCursor);
			bHandled = FALSE;
		}
...
template <class T> HCURSOR CSplitterEx< T >::m_hCursor = NULL;
template <class T> HCURSOR CSplitterEx< T >::m_vCursor = NULL;
