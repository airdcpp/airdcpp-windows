...
void WizardAutoConnectivity::on(ConnectivityManagerListener::Message, const LogMessagePtr& aMessage) noexcept {
	wizard->callAsync([this, aMessage] {
		auto msg = Text::toT(aMessage->getLabel() + ": " + aMessage->getText()) /* + _T("\n")*/;
		addLogLine(msg); 
	});
}
