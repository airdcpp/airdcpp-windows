...
void PropPageTextStyles::LoadTheme(const string& path, bool silent/* = false*/) {
	...
	if(xml.findChild("Icons")) {
		if (silent || MessageBox(CTSTRING(ICONS_IN_THEME), Text::toT(APPNAME " " + shortVersionString).c_str(), MB_YESNO | MB_ICONQUESTION | MB_DEFBUTTON2) == IDYES) {
			xml.stepIn();
	...
	if(xml.findChild("Highlights")) {
		if (silent || MessageBox(CTSTRING(HIGHLIGHTS_IN_THEME), Text::toT(APPNAME " " + shortVersionString).c_str(), MB_YESNO | MB_ICONQUESTION | MB_DEFBUTTON2) == IDYES) {
...
LRESULT PropPageTextStyles::onRestoreIcons(WORD /*wNotifyCode*/, WORD /*wID*/, HWND /*hWndCtl*/, BOOL& /*bHandled*/){
	if (MessageBox(CTSTRING(ICONS_RESTORE), Text::toT(APPNAME " " + shortVersionString).c_str(), MB_YESNO | MB_ICONQUESTION | MB_DEFBUTTON2) == IDYES) {
