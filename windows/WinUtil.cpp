...
bool WinUtil::MessageBoxConfirm(SettingsManager::BoolSetting i, const tstring& txt){
	...
	if(bCheck == BST_UNCHECKED)
		ret = ::MessageBox(WinUtil::mainWnd, txt.c_str(), Text::toT(fullVersionString).c_str(), CTSTRING(DONT_ASK_AGAIN), MB_YESNO | MB_ICONQUESTION | MB_DEFBUTTON2, bCheck);
...
void WinUtil::ShowMessageBox(SettingsManager::BoolSetting i, const tstring& txt) {
	if (SettingsManager::getInstance()->get(i)) {
		...
		::MessageBox(WinUtil::mainWnd, txt.c_str(), Text::toT(fullVersionString).c_str(), CTSTRING(DONT_SHOW_AGAIN), MB_OK | MB_ICONWARNING | MB_DEFBUTTON2, bCheck);
...
void WinUtil::showMessageBox(const tstring& aText, int icon) {
	::MessageBox(WinUtil::splash ? WinUtil::splash->m_hWnd : WinUtil::mainWnd, aText.c_str(), Text::toT(fullVersionString).c_str(), icon | MB_OK);
}

bool WinUtil::showQuestionBox(const tstring& aText, int icon, int defaultButton /*= MB_DEFBUTTON2*/) {
	return ::MessageBox(WinUtil::splash ? WinUtil::splash->m_hWnd : WinUtil::mainWnd, aText.c_str(), Text::toT(fullVersionString).c_str(), MB_YESNO | icon | defaultButton) == IDYES;
}
...
void WinUtil::removeBundle(const string& aBundleToken) {
	...
	if (aBundle) {
		...
		if(::MessageBox(0, CTSTRING_F(CONFIRM_REMOVE_DIR_BUNDLE, Text::toT(aBundle->getName())), Text::toT(fullVersionString).c_str(), MB_YESNO | MB_ICONQUESTION | MB_DEFBUTTON2) != IDYES) {
			return;
		} else {
			if (finishedFiles > 0) {
				if(::MessageBox(mainWnd, CTSTRING_F(CONFIRM_REMOVE_DIR_FINISHED_BUNDLE, finishedFiles), Text::toT(fullVersionString).c_str(), MB_YESNO | MB_ICONQUESTION | MB_DEFBUTTON2) != IDYES) {
					moveFinished = false;
				}
			}
		}
...
bool WinUtil::checkClientPassword() {
	...
	if(passDlg->DoModal(NULL) == IDOK) {
		if (passDlg->line != Text::toT(Util::base64_decode(SETTING(PASSWORD)))) {
			MessageBox(mainWnd, CTSTRING(INVALID_PASSWORD), Text::toT(fullVersionString).c_str(), MB_OK | MB_ICONERROR);
			return false;
		}
	}
