...
public:
	...
	BEGIN_MSG_MAP(DirectoryListingFrame)
		...
		COMMAND_RANGE_HANDLER(IDC_DOWNLOAD_FAVORITE_DIRS, IDC_DOWNLOAD_FAVORITE_DIRS + FavoriteManager::getInstance()->getFavoriteDirs().size() + ShareManager::getInstance()->getGroupedDirectories().size(), onDownloadFavoriteDirs)
		COMMAND_RANGE_HANDLER(IDC_DOWNLOAD_WHOLE_FAVORITE_DIRS, IDC_DOWNLOAD_WHOLE_FAVORITE_DIRS + FavoriteManager::getInstance()->getFavoriteDirs().size() + ShareManager::getInstance()->getGroupedDirectories().size(), onDownloadWholeFavoriteDirs)
	...
	public:
		...
		const tstring getText(uint8_t col) const {
			switch(col) {
				...
				case COLUMN_DATE: return (type == DIRECTORY && dir->getDirDate() > 0) ? Text::toT(Util::getDateTime(dir->getDirDate())) : Util::emptyStringT;
