	...
	public:
		...
		api_return handleSubscribeHook(ApiRequest& aRequest) {
			...
			if (BaseType::hasEntitySubscribers(subscription)) {
				throw RequestException(http::status::conflict, "Global hook subscription can't be added while ID-specific subscriptions are active");
			}
		...
		api_return handleAddHookEntity(ApiRequest& aRequest) {
			...
			if (!BaseType::filterableSubscriptionExists(apiHook.getHookId())) {
				throw RequestException(http::status::not_found, "No such filterable hook: " + apiHook.getHookId());
			}

			if (BaseType::subscriptionActive(apiHook.getHookId())) {
				throw RequestException(http::status::conflict, "ID-specific subscription can't be added while the hook is globally active");
			}
