...
#define HOOK_ADD_SOURCE "queue_add_source_hook"
	...
	BundlePtr QueueApi::getBundle(ApiRequest& aRequest) {
		...
		if (!b) {
			throw RequestException(http::status::not_found, "Bundle " + Util::toString(bundleId) + " was not found");
		}
	...
	api_return QueueApi::handleAddFileBundle(ApiRequest& aRequest) {
		...
		addAsyncTask([
			...
			complete(http::status::ok, Serializer::serializeBundleAddInfo(bundleAddInfo), nullptr);
			return;
		});
	...
	api_return QueueApi::handleAddDirectoryBundle(ApiRequest& aRequest) {
		...
		addAsyncTask([
			...
			try {
				...
				complete(http::status::bad_request, nullptr, e.toJSON());
				return;
			}
			...
			// Handle results
			if (!result) {
				complete(http::status::bad_request, nullptr, ApiRequest::toResponseErrorStr(errorMsg));
				return;
			}

			complete(http::status::ok, Serializer::serializeDirectoryBundleAddResult(*result, errorMsg), nullptr);
			return;
		});
	...
	QueueItemPtr QueueApi::getFile(ApiRequest& aRequest, bool aRequireBundle) {
		...
		if (!q) {
			throw RequestException(http::status::not_found, "File not found");
		}

		if (aRequireBundle && !q->getBundle()) {
			throw RequestException(http::status::bad_request, "This method may only be used for bundle files");
		}
	...
	Segment QueueApi::parseSegment(const QueueItemPtr& qi, ApiRequest& aRequest) {
		if (!QueueManager::getInstance()->isWaiting(qi)) {
			throw RequestException(http::status::precondition_failed, "Segments can't be modified for running files");
		}
		...
		if (s.getSize() != qi->getSize()) {
			...
			if (s.getStart() % blockSize != 0) {
				throw RequestException(http::status::bad_request, "Segment start must be aligned by " + Util::toString(blockSize));
			}

			if (s.getEnd() != qi->getSize()) {
				if (s.getEnd() > qi->getSize()) {
					throw RequestException(http::status::bad_request, "Segment end is beyond the end of the file");
				}

				if (s.getSize() % blockSize != 0) {
					throw RequestException(http::status::bad_request, "Segment size must be aligned by " + Util::toString(blockSize));
				}
			}
		}
